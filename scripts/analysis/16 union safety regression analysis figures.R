...
reg_output_no_int <- haven::read_dta(file.path(input, "15 union safety regression output - no interaction.dta")) %T>%
  dplyr::glimpse()

reg_output_cleaned_no_int <- reg_output_no_int %>%
  dplyr::filter(grepl("1.union", var) & !grepl("c.size_100FTEs", var)) %>%
  dplyr::mutate(irr = coef) %>%
  dplyr::select(lhs_var, persistence_level, year_range, irr, starts_with("ci")) %T>%
  dplyr::glimpse()
  ...
  for (persistence_level_pct in c(70, 75, 80, 90)) {
    plot_data <- reg_output_cleaned %>%
      dplyr::filter(lhs_var == lhs_var_i,
                    ...
                    !(year_range %in% c("2000_2010", "2011_2022", "2000_2022"))) %>%
    ...
    irr_plot <- ggplot(plot_data,
      ...
      scale_y_continuous(breaks = seq(0, max(plot_data$ci_upper)+0.25, 0.5),
                         limits = c(0, max(plot_data$ci_upper)+0.5)) +
      geom_text(aes("2000-2004",1,label = "IRR = 1", vjust = -0.5, hjust = 1), family = "serif", size = 8) +
      ...
      geom_point(aes(color = persistence_level, shape = persistence_level), alpha = 0.9, position = position_dodge(0.8), size = 8) +
      scale_color_manual(values = c("#018571", "#dfc27d")) +
      labs(x = NULL, 
           y = "IRR Coefs on Union Indicators (Main Effects)") +
...
for (lhs_var_i in c("traumatic_injuries", "ss_violations")) {
  for (persistence_level_pct in c(70, 75, 80, 90)) {
    plot_data <- reg_output_cleaned_no_int %>%
      dplyr::filter(lhs_var == lhs_var_i,
                    persistence_level %in% c("MINE_ID_active_anypct", stringr::str_glue("MINE_ID_active_{persistence_level_pct}pct")),
                    !(year_range %in% c("2000_2010", "2011_2022", "2000_2022"))) %>%
      dplyr::mutate(year_range = stringr::str_replace(year_range, "_", "-"),
    ...
    prop_functions$prop_save(irr_plot, file.path("../../output/figures/irr_figures_union_premium", stringr::str_glue("irr_figure_MINE_ID_active_{persistence_level_pct}pct_{lhs_var_i}_no_int")))
  }
}
